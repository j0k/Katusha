...
Мультиинсталляция позволяет управлять Катюшой:
...
- Сервер Катюшы отвечает за выполнение команд

**[клиент] Speech-To-Text (распознавание голоса)** ---> _[веб-сервер Катюшы] runva_webapi -> plugin -> Text-To-Speech (генерация голосового ответа)_
...
- Вам нужно запустить сервер Катюшы - запустите **runva_webapi.py**.
Так запустится Катюша в формате веб-сервера с REST API, к которой могут обращаться
...
- Сконфигурируйте, где находится сервер Катюшы. Настройки сервера - host,port,log_level настраиваются в ~~**options/webapi.json**~~ 
...
При необходимости вы можете запустить больше клиентов - все они будут обращаться к единому серверу Катюшы.
...
Клиент может определить, как он хочет, чтобы озвучивался результат работы Катюшы.
...
С версии 4.1 Катюшы немного поменялся формат результата вызова (для saytxt), 
...
На сервере
- Запустите `runva_webapi.py`. Это обычный WEBAPI Катюшы
...
шло по другому URL/порту, а не вместе с Webapi Катюшы. 

### Телеграм-клиент

Можно запустить Телеграм-клиент для доступа к серверу Катюшы через телеграм-бота.
